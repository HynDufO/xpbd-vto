...
def add_constraints(a, inLBS, i, j):
    if inLBS[i] and inLBS[j]:
        return
    a.append((i, j))
...
if __name__ == "__main__":
    ...
    vertices_lbs, _ = read_obj_file("starlight_lbs_3.obj")
    inLBS = [False] * len(vertices)
    for i in range(len(vertices_lbs)):
        for j in range(len(vertices)):
            ok = True
            for k in range(3):
                if abs(vertices[j][k] - vertices_lbs[i][k]) > 1e-8:
                    ok = False
                    break
            if ok:
                inLBS[j] = True
    ...
    for tri in triangles:
        add_constraints(stretching_constraints, inLBS, tri[0], tri[1])
        add_constraints(stretching_constraints, inLBS, tri[0], tri[2])
        add_constraints(stretching_constraints, inLBS, tri[1], tri[2])
    ...
    while i < len(edges_tri):
        ...
        if i < len(edges_tri) and edges_tri[i][0] == edges_tri[i - 1][0] and edges_tri[i][1] == edges_tri[i - 1][1]:
            ...
            if edges_tri[i][2] < edges_tri[i - 1][2]:
                add_constraints(bending_constraints, inLBS, edges_tri[i][2], edges_tri[i - 1][2])
                # bending_constraints.append((edges_tri[i][2], edges_tri[i - 1][2]))
            else:
                add_constraints(bending_constraints, inLBS, edges_tri[i - 1][2], edges_tri[i][2])
    ...
    output_file = obj_file_path.split('.')[0] + "_constraints_groups_exclude_lbs_3"
